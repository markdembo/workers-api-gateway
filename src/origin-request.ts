...
export const getInternalRoute =(route: Route, request: Request): string =>{
	if(route.type === "global") return route.endpoint
		...
		if(request.cf?.continent && route.regionalEndpoint[request.cf.continent as CONTINENT]){
...
export const fetchRoute = async (route: Route, request: Request) => {
	...
	if(route.urlTransformer) { urlString = route.urlTransformer(urlString) }

	// Fetch request
	const modifiedRequest = new Request(request, {headers: filterHeaders(request.headers)} )
	try {
